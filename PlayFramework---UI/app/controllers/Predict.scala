...
object Predict {
  def predict(Radiant:Array[Int],Dire:Array[Int]): String ={
...
//    }
    ...
    val countering_result = spark.sqlContext.read
      .option("header", "true")
      .option("inferSchema", "true")
      .csv("app/resources/countering.csv")
    ...
    val player_template = spark.sqlContext.read
      ...
      .option("charset", "UTF8")
      .option("inferSchema", "true")
      .csv("app/resources/player_template.csv")
    ...
    val sameModel = PipelineModel.load("app/resources/svc")
